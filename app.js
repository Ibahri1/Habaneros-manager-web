    ...
    "src/renderer/app.ts": function(require, exports) {
      ...
      function openSchedulePostedTextModal(weekStart) {
          ...
          <div class="modal-panel schedule-posted-text-modal" role="dialog" aria-modal="true" aria-labelledby="schedule-posted-text-title">
            ...
            <form data-schedule-text-form class="preferred-settings-form">
              ...
              <div class="deadline-preview">${(0, dom_1.escapeHtml)(unavailableSummary)}</div>
              <fieldset><legend>Recipients</legend><div class="schedule-text-recipient-list">
                ${eligible.length ? eligible.map((row) => '<label class="check-row schedule-text-recipient"><input data-schedule-text-worker="' + row.worker.id + '" type="checkbox" checked><span><strong>' + (0, dom_1.escapeHtml)(row.worker.name) + '</strong><em>' + (0, dom_1.escapeHtml)(scheduleTextPhoneLabel(row.phone)) + '</em></span></label>').join("") : '<div class="empty-state">No active employees have usable phone numbers.</div>'}
              </div></fieldset>
              ${unavailable.length ? '<fieldset><legend>Cannot Text</legend><div class="schedule-text-recipient-list">' + unavailable.map((row) => '<label class="check-row schedule-text-recipient unavailable"><input type="checkbox" disabled><span><strong>' + (0, dom_1.escapeHtml)(row.worker.name) + '</strong><em>' + (0, dom_1.escapeHtml)(row.phone.reason) + (row.phone.raw ? ': ' + (0, dom_1.escapeHtml)(row.phone.raw) : '') + '</em></span></label>').join("") + '</div></fieldset>' : ""}
      ...
      async function sendSchedulePostedText(event, overlay, weekStart, close) {
          ...
          if (!recipients.length) {
              showToast("Select at least one employee with a usable phone number.", "bad", 7000);
              return;
          }
          try {
              ...
              if (failureDetails.length)
                  await showDialogMessage("Schedule text sent to " + sent + " employees. Failed for " + failed + " employee" + (failed === 1 ? "" : "s") + ":\n\n" + failureDetails.join("\n"));
          }
          catch (error) {
      ...
      function normalizeSmsPhone(value) {
          ...
          if (!raw)
              return { raw, normalized: "", usable: false, reason: "Missing phone number" };
          const plusCleaned = raw.replace(/[^\d+]/g, "");
          if (/^\+1\d{10}$/.test(plusCleaned))
              return { raw, normalized: plusCleaned, usable: true, reason: "" };
          const digits = raw.replace(/\D/g, "");
          if (digits.length === 10)
              return { raw, normalized: "+1" + digits, usable: true, reason: "" };
          if (digits.length === 11 && digits.startsWith("1"))
              return { raw, normalized: "+" + digits, usable: true, reason: "" };
          return { raw, normalized: "", usable: false, reason: "Invalid phone number" };
      }
      function scheduleTextPhoneLabel(phone) {
          return phone.raw && phone.raw !== phone.normalized ? phone.raw + " -> " + phone.normalized : phone.normalized;
      }
      function parseSchedulePostedResults(result) {
          if (Array.isArray(result.results))
              return result.results.filter((row) => Boolean(row && typeof row === "object"));
